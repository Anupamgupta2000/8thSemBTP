...
@app.route("/headphone", methods=['GET'])
def headphone():
    conn = MySQLdb.connect(host= "localhost",
                  user="root",
                  passwd="Anup@210",
    ...
    #Fetch the reviews and their corresponding ratings for a product
    c.execute("SELECT * FROM headphone ORDER BY timeadded DESC")
    ...
    return render_template('headphone.html', data=data, stars = stars)
...
def mobile():
    ...
    #Fetch the reviews and their corresponding ratings for a product
    c.execute("SELECT * FROM mobile ORDER BY timeadded DESC")
    ...
    return render_template('mobile.html', data=data, stars = stars)



@app.route("/macbook", methods=['GET'])
def macbook():
    conn = MySQLdb.connect(host= "localhost",
                  user="root",
                  passwd="Anup@210",
    ...
    #Fetch the reviews and their corresponding ratings for a product
    c.execute("SELECT * FROM laptop ORDER BY timeadded DESC")
    ...
    return render_template('macbook.html', data=data, stars = stars)
...
@app.route("/postreview_1", methods=['POST'])
def postreview1():
    #Get the data posted from the form
    message = request.get_json(force=True)
    review = message['review']
    ...
    c.execute(
      """INSERT INTO headphone (uname, review, rate)
      VALUES (%s, %s, %s)""",(uname, review , value ))
    
    conn.commit()

    #Calculate average of the ratings including the recent value
    c.execute("""SELECT AVG(rate) FROM headphone""")

    avg = c.fetchone()

    #Update the overall rating of a product in database
    c.execute("""UPDATE product SET rating = %s WHERE id = %s""",(avg,2))

    conn.commit()

    return

@app.route("/postreview_2", methods=['POST'])
def postreview2():
    #Get the data posted from the form
    message = request.get_json(force=True)
    review = message['review']
    ...
    c.execute(
      """INSERT INTO mobile (uname, review, rate)
      VALUES (%s, %s, %s)""",(uname, review , value ))
    
    conn.commit()

    #Calculate average of the ratings including the recent value
    c.execute("""SELECT AVG(rate) FROM mobile""")

    avg = c.fetchone()

    #Update the overall rating of a product in database
    c.execute("""UPDATE product SET rating = %s WHERE id = %s""",(avg,3))
...
@app.route("/postreview_3", methods=['POST'])
def postreview3():
    #Get the data posted from the form
    message = request.get_json(force=True)
    review = message['review']
    ...
    c.execute(
      """INSERT INTO laptop (uname, review, rate)
      VALUES (%s, %s, %s)""",(uname, review , value ))
    
    conn.commit()

    #Calculate average of the ratings including the recent value
    c.execute("""SELECT AVG(rate) FROM laptop""")

    avg = c.fetchone()

    #Update the overall rating of a product in database
    c.execute("""UPDATE product SET rating = %s WHERE id = %s""",(avg,4))
